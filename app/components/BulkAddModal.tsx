...
interface BulkAddModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (rawCodes: string[]) => Promise<{ success: number; notFound: string[] }>;
}

export default function BulkAddModal({ isOpen, onClose, onSave }: BulkAddModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setResult(null);
    try {
      const res = await onSave(rawList);
      setResult(res);
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
          <div>
            ...
            <textarea
              ...
              className="w-full h-32 bg-surface-container p-4 border border-outline-variant/50 rounded-xl text-on-surface font-mono placeholder-on-primary-container outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition-all resize-none"
            />
          ...
          {result && (
            <div className={`p-4 rounded-xl border ${
              ...
              <div className="flex items-center gap-2 font-bold">
                ...
                {result.success > 0 
                  ? `${result.success} figurinhas adicionadas com sucesso!` 
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={handleClose}
              className="px-5 py-2.5 text-label-md font-semibold text-on-surface-variant hover:bg-surface-container-high rounded-xl transition-colors"
            >
            ...
            <button
              ...
              className="px-6 py-2.5 text-label-md font-bold bg-secondary text-on-secondary hover:opacity-90 active:scale-[0.98] rounded-xl transition-all flex items-center gap-2 shadow-lg disabled:opacity-40 disabled:pointer-events-none glow-blue"
            >
              {isSaving ? (
                <>
